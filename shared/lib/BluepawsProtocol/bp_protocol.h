/*
  ┌─────────────────────────────────────────────────────────────┐
  │  BLUEPAWS V4 — TLV TELEMETRY PROTOCOL v1.1                 │
  │  Shared collar/home-hub binary packet contract              │
  │  Canonical source: docs/TLV_PROTOCOL_V1_1.md                │
  └─────────────────────────────────────────────────────────────┘

  Uplink packet layout:
    [32-byte fixed header] + [0-24 bytes TLV] + [8-byte auth tag]
    Minimum: 40 bytes (header + auth tag, no TLV)
    Maximum: 64 bytes (header + 24 TLV + auth tag)

  There is no application CRC-16. The LoRa PHY CRC remains the radio-layer
...
enum bp_status_t : uint8_t {
    ...
    // Compatibility aliases while older sketch code is phased out.
...
// Older downlink/ACK helpers still use these names. They now map to tx_reason
// classes rather than an old packet-type bitfield inside flags.
...
static inline bp_profile_t bp_profile_from_name(const char *name) {
    ...
    if (strcmp(name, "power_save") == 0)     return PROFILE_POWERSAVE;
...
// Initialize v1.1 header. Zeroes the full buffer first.
static inline void pkt_init(uint8_t *buf, uint16_t device_id,
    ...
    memcpy(&buf[3], &msg_seq,   2);
    memcpy(&buf[5], &time_unix, 4);
    buf[9]  = bp_pack_state(status, power_profile);
    buf[10] = flags;
    buf[11] = tx_reason;
}

// Compatibility overload for old call sites. Prefer the explicit v1.1 overload.
static inline void pkt_init(uint8_t *buf, uint16_t device_id,
                            uint32_t msg_seq, uint32_t time_unix,
                            uint8_t status, uint8_t tx_reason) {
    pkt_init(buf, device_id, (uint16_t)(msg_seq & 0xFFFF), time_unix,
             status, PROFILE_NORMAL, 0, tx_reason);
}

static inline void pkt_set_gps(uint8_t *buf, int32_t lat_e7, int32_t lon_e7) {
    memcpy(&buf[12], &lat_e7, 4);
    memcpy(&buf[16], &lon_e7, 4);
}

static inline void pkt_set_quality(uint8_t *buf, uint16_t batt_mV,
                                   uint16_t acc_m, uint16_t fix_age_s) {
    memcpy(&buf[20], &batt_mV,    2);
    memcpy(&buf[22], &acc_m,      2);
    memcpy(&buf[24], &fix_age_s,  2);
}

static inline void pkt_set_sat_count(uint8_t *buf, uint8_t sat_count) {
    buf[26] = sat_count;
}
...
static inline bool pkt_add_raw_tlv(uint8_t *buf, uint8_t type, const uint8_t *val, uint8_t len) {
    ...
    if ((uint16_t)tlv_len + 2 + len > BP_MAX_TLV_SIZE)
        return false;
    buf[off] = type;
    buf[off + 1] = len;
...
static inline bool pkt_add_tlv_u8(uint8_t *buf, uint8_t type, uint8_t val) {
    return pkt_add_raw_tlv(buf, type, &val, 1);
}
...
static inline bool pkt_add_tlv_u16(uint8_t *buf, uint8_t type, uint16_t val) {
    uint8_t raw[2];
    memcpy(raw, &val, 2);
    return pkt_add_raw_tlv(buf, type, raw, 2);
}

static inline bool pkt_add_tlv_u32(uint8_t *buf, uint8_t type, uint32_t val) {
    uint8_t raw[4];
    memcpy(raw, &val, 4);
    return pkt_add_raw_tlv(buf, type, raw, 4);
}

static inline bool pkt_add_tlv_log_info(uint8_t *buf, uint16_t entries, uint16_t size_kb) {
    uint8_t raw[4];
    memcpy(&raw[0], &entries, 2);
    memcpy(&raw[2], &size_kb, 2);
    return pkt_add_raw_tlv(buf, TLV_LOG_INFO, raw, 4);
}

// Finalize: append an 8-byte auth tag. This placeholder keeps firmware packet
// sizing/layout correct while the embedded HMAC-SHA256 implementation is wired.
...
static inline uint16_t pkt_msg_seq(const uint8_t *b)       { uint16_t v; memcpy(&v, &b[3],  2); return v; }
static inline uint32_t pkt_time_unix(const uint8_t *b)     { uint32_t v; memcpy(&v, &b[5],  4); return v; }
static inline uint8_t  pkt_state(const uint8_t *b)         { return b[9]; }
static inline uint8_t  pkt_status(const uint8_t *b)        { return b[9] & 0x0F; }
static inline uint8_t  pkt_power_profile(const uint8_t *b) { return (b[9] >> 4) & 0x0F; }
static inline uint8_t  pkt_flags(const uint8_t *b)         { return b[10]; }
static inline uint8_t  pkt_tx_reason(const uint8_t *b)     { return b[11]; }
static inline int32_t  pkt_lat_e7(const uint8_t *b)        { int32_t  v; memcpy(&v, &b[12], 4); return v; }
static inline int32_t  pkt_lon_e7(const uint8_t *b)        { int32_t  v; memcpy(&v, &b[16], 4); return v; }
static inline uint16_t pkt_batt_mV(const uint8_t *b)       { uint16_t v; memcpy(&v, &b[20], 2); return v; }
static inline uint16_t pkt_acc_m(const uint8_t *b)         { uint16_t v; memcpy(&v, &b[22], 2); return v; }
static inline uint16_t pkt_fix_age_s(const uint8_t *b)     { uint16_t v; memcpy(&v, &b[24], 2); return v; }
static inline uint8_t  pkt_sat_count(const uint8_t *b)     { return b[26]; }
...
// Validate v1.1 structure. Full HMAC validation is done by Supabase ingestion.
static inline bool pkt_validate_structure(const uint8_t *buf, uint8_t total_len) {
    if (total_len < BP_MIN_PACKET_SIZE || total_len > BP_MAX_PACKET_SIZE)
        return false;
    if (buf[0] != BP_PROTOCOL_VERSION)
        return false;
    uint8_t tlv_len = buf[31];
    if (tlv_len > BP_MAX_TLV_SIZE)
        return false;
    if (total_len != BP_HEADER_SIZE + tlv_len + BP_AUTH_TAG_SIZE)
        return false;
...
// Compatibility name retained for older firmware call sites.
static inline bool pkt_validate_crc(const uint8_t *buf, uint8_t total_len) {
    return pkt_validate_structure(buf, total_len);
}
