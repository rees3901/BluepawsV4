...
export function buildHubReport(hub: HubPresence): Pick<DeviceReport,"rows"|"summary"> {
  ...
  const battery = hub.battery_percent === null || hub.battery_percent === undefined
    ? "No data"
    : `${hub.battery_percent}%${hub.battery_simulated ? " (simulated testbed)" : ""}`;
  const values = [
    ...
    ["Coordinates",coordinates,hub.position_simulated ? "Temporary simulated testbed position within five metres of the configured home point." : "The hub’s own last known GPS position, never a collar’s position."],
    ["GPS fix",hub.fix_at ? new Date(hub.fix_at).toLocaleString() : "Not acquired",hub.position_simulated ? "Generated by the temporary P4 daughterboard test runtime." : "When the position was measured; this may be older than the last contact."],
    ["Battery",battery,hub.battery_simulated ? "Temporary simulated testbed level; no battery sensor supplied this value." : "The hub’s most recent battery reading."],
