...
(function (root) {
    ...
    function reportRows(s) {
        return [
            ...
            ['Coordinates',Number.isFinite(s.latitude) && Number.isFinite(s.longitude) ? s.latitude.toFixed(5)+', '+s.longitude.toFixed(5) : 'Waiting for GPS fix',s.position_source === 'starter' ? 'Starter map position until the hub has a real location.' : 'The hub’s own GPS position, never a collar’s position.'],
            ['GPS fix',s.position_source === 'starter' ? 'Starter location' : s.fix_age_s == null ? 'Not acquired' : s.fix_age_s+' seconds old','Position age is separate from last contact.'],
