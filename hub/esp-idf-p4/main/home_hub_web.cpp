...
{
    const WebSnapshot state = snapshot();
    const uint32_t uptime_seconds = static_cast<uint32_t>(esp_timer_get_time() / 1000000);
    ...
    cJSON_AddStringToObject(json, "position_source",
                            telemetry.position_simulated ? "simulated" : "gnss");
